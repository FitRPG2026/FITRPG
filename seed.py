...
from sqlalchemy import (
    ...
    DateTime, Date, Text, Numeric, ForeignKey
)
...
class UserAuth(Base):
    ...
    current_login_streak_days = Column(Integer, default=0)
    updated_at = Column(DateTime(timezone=True))
...
class Challenge(Base):
    ...
    challenge_type = Column(String(60), nullable=False)
    goal_value = Column(Numeric(12,2), nullable=False)
    reward_exp = Column(Integer, default=0)
    start_date = Column(DateTime(timezone=True))
    end_date = Column(DateTime(timezone=True))
    mechanic_type = Column(String(30), nullable=False)
    event_trigger = Column(String(60), nullable=False)
...
def seed_user_auth(session, user_id):
    print("-> seeding user_auth")

    stmt = insert(UserAuth).values(
        user_id=user_id,
...
def main():
    print("🌱 Starting FitRPG seed...")

    session = Session()

    uid = seed_users(session)
    seed_user_auth(session, uid)
...
# # Updated seed_users to match the "users" table in initial schema
# # def seed_users(conn):
# #     print("  -> seeding users...")
# #     users = [
# #         ("anna.nowak@fitrpg.dev", "active"),
# #         ("adam.kowalski@fitrpg.dev", "active"),
# #         ("karol.testowy@fitrpg.dev", "active"),
# #     ]
# #     uid_map = {}
# #     for email, status in users:
# #         # password_hash is no longer in this table
# #         row = conn.execute(
# #             text("""
...
# # # Updated to handle password storage in user_auth
# # def seed_user_auth(conn, uid):

# #     print("  -> seeding user_auth...")
# #     # Map of user email to their plain-text password for seeding
# #     auth_data = [
# #         ("anna.nowak@fitrpg.dev", "123"),
# #         ("adam.kowalski@fitrpg.dev", "123"),
# #         ("karol.testowy@fitrpg.dev", "123"),
# #     ]
# #     for email, password in auth_data:
# #         pw_hash = hash_pw(password)
# #         conn.execute(
# #             text("""
# #                 INSERT INTO user_auth (user_id, password_hash, password_updated_at, )
# #                 VALUES (:uid, :p, NOW(),NOW())
# #                 ON CONFLICT (user_id) DO UPDATE SET password_hash = :p, updated_at = NOW()
# #             """),
# #             {"uid": uid[email], "p": pw_hash},
# #         )

# # # Ensure profile links to the user_id correctly
# # def seed_user_profiles(conn, uid):
# #     print("  -> seeding user_profiles...")
# #     profiles = [
# #         ("anna.nowak@fitrpg.dev", "annanowak", "Anna Nowak", date(1995, 5, 20)),
# #         ("adam.kowalski@fitrpg.dev", "adamk", "Adam Kowalski", date(1988, 11, 2)),
# #     ]
# #     for email, username, dname, bday in profiles:
# #         conn.execute(
# #             text("""
# #                 INSERT INTO user_profiles (user_id, username, display_name, birth_date, updated_at)
# #                 VALUES (:uid, :u, :d, :b, NOW())
# #                 ON CONFLICT (user_id) DO UPDATE 
# #                 SET username = :u, display_name = :d, updated_at = NOW()
# #             """),
# #             {"uid": uid[email], "u": username, "d": dname, "b": bday},
# #         )

# # # Ensure progress links to the user_id correctly
# # def seed_user_progress(conn, uid):
# #     print("  -> seeding user_progress...")
# #     for email, user_id in uid.items():
# #         conn.execute(
# #             text("""
# #                 INSERT INTO user_progress (user_id, total_exp, level, updated_at)
# #                 VALUES (:uid, 0, 1, NOW())
